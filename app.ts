...
class ThreeJSContainer {
    ...
    private createScene = () => {
        ...
        for (let row = 0; row < 5; row++) {
            ...
            const cueBallPosition = middleSpherePosition.clone();
            cueBallPosition.z = -middleSpherePosition.z;

            const redMaterial = new THREE.MeshLambertMaterial({ color: 0xff0000 });

            const newSphere = new THREE.Mesh(geometry, redMaterial);
            newSphere.position.copy(cueBallPosition);
            this.scene.add(newSphere);
            this.sphereMeshes.push(newSphere);

            const newSphereShape = new CANNON.Sphere(1);
            const cueBall = new CANNON.Body({ mass: 10 });
